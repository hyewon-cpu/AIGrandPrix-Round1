...
DEFAULT_MODEL_PATH = os.path.join(
    SCRIPT_DIR, "race_model", "run_7", "checkpoint0002.pth"
)
...
class LearnedAccelModel:
    def __init__(self, control_model_path, dim_obs=10, dim_action=6, device="cpu"):
        ...
        checkpoint = torch.load(control_model_path, map_location=self.device)
...
class DiffSimRacer:
    def __init__(
        ...
        self.current_depth_frame_id = 0
        self.last_depth_frame_id = -1
        self._depth_last_fps = None
        self._depth_last_frame_id = 0
        ...
        self.last_segmentation_center_px = None
        self.last_segmentation_backup_center_px = None
        ...
        # Frame ids to help keep RGB/depth aligned when syncing control to depth.
        self.current_rgb_frame_id = 0
        # Per-control-iteration snapshot to ensure downstream target estimators
        # can use the RGB that corresponds to the depth frame the control loop uses.
        self._control_rgb_response_snapshot = None
        ...
        self.control_loop_thread = threading.Thread(
            ...
            args=(self.control_callback, control_sleep),
        )
    ...
    def segmentation_candidate_to_target_point_airsim(
        ...
        if candidate is None:
            return None, np.inf, "unavailable", None, None
        ...
        target_vec_airsim_world = camera_rot @ target_rel_camera
        p_target_airsim = camera_position + target_vec_airsim_world
        return (
            p_target_airsim,
            depth_m,
            depth_source,
            target_vec_airsim_world.astype(np.float32),
    ...
    def get_cached_segmentation_target_point_airsim(self):
        if self.last_segmentation_target_airsim is not None:
            return self.last_segmentation_target_airsim, {
                ...
                "segmentation_center_px": getattr(self, "last_segmentation_center_px", None),
                "segmentation_target_vec_airsim_world": getattr(
                    self, "last_segmentation_target_vec_airsim_world", None
                ),
            }
        if self.last_segmentation_backup_target_airsim is not None:
            return self.last_segmentation_backup_target_airsim, {
                ...
                "segmentation_center_px": getattr(
                    self, "last_segmentation_backup_center_px", None
                ),
                "segmentation_target_vec_airsim_world": getattr(
                    self, "last_segmentation_backup_target_vec_airsim_world", None
                ),
            }
        return None, {}
    ...
    def estimate_segmentation_target_point_airsim(self):
        ...
        candidates = self.extract_segmentation_candidates_from_mask(
            mask, depth_image=self.current_depth
        )
        if not candidates:
            return self.get_cached_segmentation_target_point_airsim()
        ...
        (
            ...
            selected_target_vec_airsim_world,
            selected_target_rel_camera,
        ) = self.segmentation_candidate_to_target_point_airsim(
            selected_candidate,
            intr,
        ...
        (
            ...
            backup_depth_m,
            backup_depth_source,
            backup_target_vec_airsim_world,
            backup_target_rel_camera,
        ) = self.segmentation_candidate_to_target_point_airsim(
            backup_candidate,
            intr,
            self.last_segmentation_response,
        ...
        if backup_target_airsim is None:
            ...
            backup_target_vec_airsim_world = selected_target_vec_airsim_world
            backup_target_rel_camera = selected_target_rel_camera
        ...
        self.last_segmentation_target_vec_airsim_world = selected_target_vec_airsim_world
        self.last_segmentation_backup_target_vec_airsim_world = backup_target_vec_airsim_world
        self.last_segmentation_center_px = np.asarray(
            selected_candidate["center"], dtype=np.float32
        ).copy()
        self.last_segmentation_backup_center_px = np.asarray(
            backup_candidate["center"], dtype=np.float32
        ).copy()
        self.last_segmentation_candidate_targets_airsim = [
            selected_target_airsim,
            backup_target_airsim,
        ...
        aux = {
            ...
            "segmentation_target_vec_airsim_world": selected_target_vec_airsim_world,
            "segmentation_backup_target_vec_airsim_world": backup_target_vec_airsim_world,
            "segmentation_target_rel_camera": selected_target_rel_camera,
    ...
    def ground_truth_gate_target_point_airsim(self, position_airsim):
        ...
        active_idx = min(self.active_gate_index, len(self.gate_poses_ground_truth) - 1) #clamp the gate index so it never goes past the last available gate 
        #self.active_gate_index is the gate the drone is currently trying to reach 
        active_gate_position = self.gate_poses_ground_truth[active_idx].position #active gate position
        gate_pos_airsim = np.array( #convert gate position into numpy
            [active_gate_position.x_val, active_gate_position.y_val, active_gate_position.z_val], dtype=np.float32
        )
        ...
        if distance < 2.0 and active_idx < len(self.gate_poses_ground_truth) - 1:
            ...
            active_gate_position = self.gate_poses_ground_truth[active_idx].position
            gate_pos_airsim = np.array( #convert gate position into numpy
                [active_gate_position.x_val, active_gate_position.y_val, active_gate_position.z_val], dtype=np.float32
            )
    ...
    def preprocess_depth(self, depth):
        depth = depth.copy()
        depth[~np.isfinite(depth)] = 24.0 #replace invalid values with 24 
        depth[depth <= 0.0] = 24.0 #replace minuses with 24
        depth = 3.0 / np.clip(depth, 0.3, 24.0) - 0.6 #clip depth to -0.3 -24, and divide by 3 so that it's flipped. 
        h, w = depth.shape
        ...
        depth_tensor = torch.as_tensor(depth, dtype=torch.float32)[None, None] #add 2 new empty dimensions in the front 
    ...
    def image_callback(self):
        ...
        with self._sensor_cond:
            if depth is not None:
                ...
                self.current_depth_frame_id += 1
                frame_id = int(self.current_depth_frame_id)
                depth_updated = True
                if self.debug_print and (self.current_depth_frame_id % self.debug_print_every == 0):
                    if self._depth_last_fps is not None:
                        dt = float(now - self._depth_last_fps)
                        frames = int(self.current_depth_frame_id - self._depth_last_frame_id)
                        if dt > 1e-6 and frames > 0:
                            fps = frames / dt
                            print("[sensor]", "depth_fps=", round(fps, 2), "frame_id=", self.current_depth_frame_id)
                    self._depth_last_fps = float(now)
                    self._depth_last_frame_id = int(self.current_depth_frame_id)
            if rgb is not None:
                self.current_rgb_frame_id += 1
        ...
        if rgb is not None:
            if self.viz_rgb and cv2 is not None:
                ...
                if hasattr(self, "swap_rb") and getattr(self, "swap_rb"):
                    ...
                    rgb_viz = rgb_viz[..., ::-1]
                rgb_viz = self.augment_rgb_viz(rgb_viz) #returns same image with added annotations
    ...
    def augment_rgb_viz(self, rgb_viz):
        ...
        last_src = str(getattr(self, "last_target_source", ""))
        if last_src.startswith("segmentation_cache_rank_"):
            cache_used = True
            try:
                cache_rank = int(last_src.split("_")[-1])
            except Exception:
                cache_rank = None

        if self.last_segmentation_response is not None:
            width = int(self.last_segmentation_response.width)
            height = int(self.last_segmentation_response.height)
            if rgb_viz.shape[1] == width and rgb_viz.shape[0] == height:
                # If segmentation is currently stale/unavailable, we still want to show the
                # last utilized (cached) center.
                center_px = getattr(self, "last_segmentation_center_px", None)
        ...
        # Draw a crosshair + small circle at the utilized segmentation center.
        color = (0, 255, 255)  # BGR yellow
        cv2.circle(rgb_viz, (x, y), 6, color, 2)
        cv2.line(rgb_viz, (x - 10, y), (x + 10, y), color, 1)
        cv2.line(rgb_viz, (x, y - 10), (x, y + 10), color, 1)

        # If we recently used cached segmentation, indicate it.
        if cache_used:
            label = "seg_center (cached)" if cache_rank is None else f"seg_center (cached {cache_rank})"
        else:
            label = "seg_center"
        cv2.putText(
            rgb_viz,
    ...
    def build_state_tensor(self, state_dict):
        ...
        #position
        position_airsim = state_dict.get("position")
        position = (
            airsim_to_normal_vector(position_airsim)
            if position_airsim is not None
            else np.zeros(3, dtype=np.float32)
        )
        #orientation
        orientation_airsim = state_dict["orientation"]
        env_rot_airsim = quaternion_to_rotation_matrix(orientation_airsim)
        env_rot = airsim_to_normal_rotation(env_rot_airsim)

        #linear velocity 
        linear_velocity_airsim = state_dict["linear_velocity"]
        linear_velocity = airsim_to_normal_vector(linear_velocity_airsim)
        
        
        #make yaw only rotation matrix 
        forward = env_rot[:, 0].copy()
        forward[2] = 0.0
        if np.linalg.norm(forward) < 1e-6: #if forward is invalid
            forward = self.current_forward.copy() #reuse last valid forward direction 
        ...
        p_target_airsim = None #target position in airsim coordintes
        p_target = None #target position in normal coordinates
        target_v = None #target vector 
        segmentation_aux = {} #
 
        if self.target_source == "ground_truth" and self.gate_poses_ground_truth:
            p_target_airsim = self.ground_truth_gate_target_point_airsim(position_airsim) #drone position 을 주면 ground truth target position 을 줌 
            self.last_target_source  = "ground_truth_gate"
        elif self.target_source == "segmentation":
        ...
        target_vec_airsim_world = segmentation_aux.get("segmentation_target_vec_airsim_world")
        if target_vec_airsim_world is not None:
            target_v = airsim_to_normal_vector(target_vec_airsim_world)
            target_v_norm = np.linalg.norm(target_v)
            if target_v_norm > 1e-6:
                target_v = target_v / target_v_norm * self.target_speed
            else:
                target_v = None
        elif p_target_airsim is not None and position_airsim is not None:
            p_target= airsim_to_normal_vector(p_target_airsim)
            target_v = p_target - position
            target_v_norm = np.linalg.norm(target_v)
            if target_v_norm > 1e-6:
                target_v = target_v / target_v_norm * self.target_speed
            else:
        ...
        aux = {
            "position": position,
            "position_airsim": position_airsim,
            "env_rot": env_rot,
            "env_rot_airsim": env_rot_airsim,
            "local_velocity": linear_velocity @ yaw_only_rot,
            "p_target": p_target, #target coordinate in normal world coordinates
            "p_target_airsim": p_target_airsim,
            "target_vec_airsim_world": (
                None
                if (p_target_airsim is None and target_vec_airsim_world is None)
                else (
                    (p_target_airsim - position)
                    if (p_target_airsim is not None and position is not None)
                    else target_vec_airsim_world
                )
            ),
            "target_v": target_v,
        }
    ...
    def infer_acceleration(self, depth, state_dict):
        ...
        state_tensor, yaw_only_rot, aux = self.build_state_tensor(state_dict) #state_dict : #position, orientation, linear velocity 
        act = self.model.predict_action(depth_tensor, state_tensor)
        act = yaw_only_rot @ act.reshape(3, -1)
        a_pred = act[:, 0] - act[:, 1]
        aux["acc_set"] = a_pred.copy()
        target_v = aux["target_v"]
        return a_pred.astype(np.float32), target_v.astype(np.float32), aux

    def acceleration_to_attitude_command(self, a_pred, aux):
        a_setpoint = a_pred.astype(np.float32)
        a_setpoint[2] += self.gravity
        thrust = float(np.linalg.norm(a_setpoint))
        if thrust < 1e-6: # thrust is too small 
            current_forward = aux["env_rot"][:, 0]
            yaw = math.atan2(float(current_forward[1]), float(current_forward[0]))
            return 0.0, 0.0, yaw, float(self.hover_throttle), thrust #keep current yaw, use hover throttle. 

        up_vec = a_setpoint / thrust
        throttle = thrust + float(
            aux["local_velocity"][2] * abs(aux["local_velocity"][2]) * 0.01
        )

        if aux["p_target"] is not None:
            target_delta = aux["p_target"] - aux["position"]
        else:
            target_delta = aux["target_v"]
        forward_vec = aux["env_rot"][:, 0] * 5.0 + target_delta
    ...
    def control_callback(self):
            ...
            with self._sensor_cond:
                while True:
                    has_new_depth = (
                        ...
                        and self.current_depth_frame_id != self.last_depth_frame_id
                    )
                    if has_new_depth:
                        break
                ...
                depth_frame_id = self.current_depth_frame_id
        ...
        self.current_state = self.fetch_state() #position, orientation, linear velocity 
        ...
        if (not self.sync_control_to_depth) and depth_frame_id == self.last_depth_frame_id: #depth not updated
            if self.debug_print and (self.debug_counter % self.debug_print_every == 0):
                print(
                    "[debug]",
                    "target_src=",
        ...
        try:
            a_pred, target_v, aux = self.infer_acceleration(
                depth, self.current_state
            )
            self.overlay = self.build_viz_overlay(aux, int(depth_frame_id))
            ...
            if self.debug_print and (self.debug_counter % self.debug_print_every == 0):
                print(
                    ...
                    "target_v=", np.round(target_v, 3), #normal coordinate
                    "acc_set=", np.round(aux.get("acc_set", np.zeros(6, dtype=np.float32)), 3),
...
def main():
    ...
    parser.add_argument("--hover_throttle", type=float, default=0.8)
    parser.add_argument("--control_period", type=float, default=0.05)
    parser.add_argument("--control_sleep", type=float, default=0.1)
    ...
    racer = DiffSimRacer(
        ...
        control_sleep = args.control_sleep,
