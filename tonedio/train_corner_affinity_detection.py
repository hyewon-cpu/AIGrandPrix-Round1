...
DEFAULT_SPLIT_DIR = Path(__file__).resolve().parents[1] / "data"/ "dataforuse"
DEFAULT_OUTPUT_DIR = Path(__file__).resolve().parents[1] / "results"/ f"run_{time}"
DEFAULT_CHECKPOINT_DIR = Path(__file__).resolve().parents[1] /"results" / "run_224224_inputedited" /"checkpoints"/"best.pt"
...
class CornerPafMSELoss(nn.Module):
    def __init__(self, *, corner_positive_weight: float = 0.0, paf_positive_weight: float = 0.0):
        super().__init__()
        self.corner_positive_weight = float(corner_positive_weight)
        self.paf_positive_weight = float(paf_positive_weight)
    ...
    def forward(
        ...
        if self.paf_positive_weight > 0:
            paf_mask = (paf_targets.abs() > 0).to(dtype=paf_preds.dtype)
            paf_weights = 1.0 + self.paf_positive_weight * paf_mask
            paf_loss = ((paf_preds - paf_targets) ** 2 * paf_weights).sum() / paf_weights.sum().clamp_min(1e-6)
        else:
...
def train_one_epoch(
    ...
    *,
    ...
    model.train() #base class of torch.nn.Module, which CornerUNet inherits from 
    total_loss = 0.0
    total_count = 0
...
def match_edges_hungarian(score_matrix: np.ndarray, *, min_score: float) -> list[tuple[int, int, float]]:
...
def score_and_match_edges(
    ...
    *,
    ...
    for edge_idx, (k, l) in enumerate(EDGE_TYPES):
        ...
        matches = match_edges_hungarian(score_matrix, min_score=edge_min_score)
...
def save_inference_overlay(
    ...
    overlays_dir = output_dir / "inference" / "overlays"
    ...
    # Draw all corner candidates as small dots (per-corner color).
    if isinstance(candidates, dict) and candidates:
        corner_colors = {
            "TL": (255, 0, 255),  # magenta
            "TR": (255, 255, 0),  # cyan-ish (BGR)
            "BL": (0, 255, 255),  # yellow
            "BR": (255, 0, 0),    # blue
        }
        for corner_name, items in candidates.items():
            if not isinstance(items, list):
                continue
            base = corner_colors.get(str(corner_name), (255, 255, 255))
            for cand in items:
                try:
                    x = float(cand.get("x"))
                    y = float(cand.get("y"))
                ...
                dot_color = (
                    int(round(base[0] * strength)),
                    int(round(base[1] * strength)),
                    int(round(base[2] * strength)),
                )
                cv2.circle(
                    overlay,
...
def render_paf_overlay(
    ...
    *,
    ...
    """Overlay PAF vector fields on an image for debugging.

    `paf_maps` is expected to be (8, H, W): 4 edges * (vx, vy).
    """
    if paf_maps.ndim != 3 or paf_maps.shape[0] != 8:
        raise ValueError(f"Expected paf maps with shape (8,H,W), got {paf_maps.shape}")
    overlay = image_bgr.copy()
    ...
    # BGR colors per edge type (TL->TR, TR->BR, BR->BL, BL->TL).
    colors = [(0, 0, 255), (0, 165, 255), (0, 255, 0), (255, 0, 0)]

    for edge_idx, _edge in enumerate(EDGE_TYPES):
        paf_xy = paf_maps[edge_idx * 2 : edge_idx * 2 + 2]
        color = colors[edge_idx % len(colors)]
        for y in range(stride // 2, h, stride):
            for x in range(stride // 2, w, stride):
                vx = float(paf_xy[0, y, x])
                vy = float(paf_xy[1, y, x])
                if (not np.isfinite(vx)) or (not np.isfinite(vy)):
                    continue
                mag = float(math.hypot(vx, vy))
                if mag < 1e-3:
                    continue
                # Dot visualization: show where the PAF is "active" without
                # encoding direction (less clutter than arrows).
                # Scale dot brightness a bit by magnitude for quick intuition.
                strength = float(np.clip(mag, 0.0, 1.0))
                dot_color = (
                    int(round(color[0] * strength)),
                    int(round(color[1] * strength)),
                    int(round(color[2] * strength)),
                )
                cv2.circle(overlay, (int(x), int(y)), 1, dot_color, -1, lineType=cv2.LINE_AA)
    return overlay


@torch.no_grad()
def save_training_gt_paf_overlays(
    loader: DataLoader,
    output_dir: Path,
    *,
    epoch: int,
    ...
    """Save a few PNGs per epoch showing GT PAF targets overlaid on input images."""
    max_images = int(max_images)
    if max_images <= 0:
        return

    overlays_dir = output_dir / "train_debug" / "gt_paf_overlays"
    overlays_dir.mkdir(parents=True, exist_ok=True)

    saved = 0
    global_idx = 0
    for images, _corner_targets, paf_targets in loader:
        images_cpu = images.detach().float().cpu().numpy()
        paf_gt = paf_targets.detach().float().cpu().numpy()

        batch_size = int(images_cpu.shape[0])
        for bi in range(batch_size):
            if saved >= max_images:
                return
            image_rgb = np.transpose(images_cpu[bi], (1, 2, 0))
            image_rgb_u8 = np.clip(image_rgb * 255.0, 0.0, 255.0).astype(np.uint8)
            image_bgr = cv2.cvtColor(image_rgb_u8, cv2.COLOR_RGB2BGR)
            overlay = render_paf_overlay(
                image_bgr,
                paf_gt[bi],
                stride=stride,
                arrow_scale=arrow_scale,
            )
            out_path = overlays_dir / f"epoch_{epoch + 1:04d}_img_{global_idx:06d}.png"
...
def run_inference(
    ...
    *,
    ...
    if output_dir:
        output_dir = Path(__file__).resolve().parents[1]/"results"/output_dir
    else:
    ...
    for image_path, _json_path in pairs:
        ...
        save_inference_overlay(output_dir, image_path.stem, image_bgr, gates, candidates)
...
def build_args():
    ...
    parser.add_argument("--split_dir", type=str, default=DEFAULT_SPLIT_DIR, help="Folder containing train.txt, val.txt, and test.txt.")
    parser.add_argument("--mode", type=str, choices=["train", "test", "inference"], default="train")
    parser.add_argument("--output_dir", type=str, default=None) #run name for saving checkpoints and inference results 
    parser.add_argument("--checkpoint", type=str, default=None) #run name for loaidng checkpoints 
    ...
    parser.add_argument("--paf_distance_px", type=float, default=10.0) #used  when making ground truth paf heatmaps
    ...
    parser.add_argument("--corner_positive_weight", type=float, default=10.0,
        help="Optional extra weight applied to nonzero corner heatmap regions (paper uses 0.0).",)
    parser.add_argument( "--paf_positive_weight",type=float, default=10.0,
        help="Optional extra weight applied to nonzero PAF target regions (default 0.0 keeps uniform MSE).",)
    ...
    # Training-time debug visualization.
    parser.add_argument(
        "--save_gt_paf_overlays",
        type=int,
        default=10,
        help="If >0, save this many PNGs per epoch with GT PAF targets overlaid (from the val loader).",
    )
    parser.add_argument(
        "--paf_overlay_stride",
        type=int,
        default=0,
        help="Grid stride (pixels) for drawing PAF arrows in the overlay.",
    )
    parser.add_argument(
        "--paf_overlay_scale",
        type=float,
        default=1.0,
        help="Arrow length multiplier for drawing PAF vectors in the overlay.",
    )
...
def main():
    ...
    if args.mode == "train":
        if args.output_dir:
            output_dir = Path(__file__).resolve().parents[1]/ "results" / args.output_dir
            output_checkpoint = output_dir / "checkpoints"
        ...
        criterion = CornerPafMSELoss(
            corner_positive_weight=args.corner_positive_weight,
            paf_positive_weight=args.paf_positive_weight,
        )
        use_amp = device.type == "cuda" #if use cuda, use use_amp, which is an automatic precision thing 
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            if int(getattr(args, "save_gt_paf_overlays", 0)) > 0:
                save_training_gt_paf_overlays(
                    val_loader,
                    output_dir,
                    epoch=epoch,
                    max_images=int(args.save_gt_paf_overlays),
                    stride=int(args.paf_overlay_stride),
                    arrow_scale=float(args.paf_overlay_scale),
                )
        ...
        checkpoint_path = Path(__file__).resolve().parents[1] /"results" / args.checkpoint /"checkpoints"/"best.pt" if args.checkpoint else DEFAULT_CHECKPOINT_DIR
